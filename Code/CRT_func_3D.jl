"""
Julia tools to create grids, compute timescales, and construct matrices for the 3D Cosmic-ray transport equation solver in cylindrical geometry and log-spaced grids.
...
include("CRT_tools_3D.jl")


# Creates logarithmically-spaced grids for radial (r), axial (z), and momentum (p) dimensions
# Inputs: nr = number of radial grid points, nz = number of axial grid points (=2n+1 where there are n points below and above 0 and 0), np = number of momentum grid points, r_min/max = min/max radial values, z_min/max = min/max axial values (positive), p_min/max = min/max momentum values
# Outputs: r_list = radial grid points, z_list = axial grid points, p_list = momentum grid points, pu_list = upper momentum grid points (p_k+1)
...
# Generates a list of timesteps decreasing geometrically from maximum to minimum
# Inputs: Dt_min = minimum timestep, m = number of times each timestep is repeated, factor = geometric factor for decreasing timesteps, nmax = multiplier to determine maximum timestep as nmax * Dt_min
# Outputs: Dt_list = list of timesteps, tot_phys_time = total physical time covered
function make_dt_list(Dt_min, m; factor=1.1, nmax=1e4)
    ...
    while Delt > Dt_min
        tot_phys_time += m * Delt
        push!(Dt_list, Delt)
        Delt /= factor
    end

    println("Total physical time = ", tot_phys_time)
    nt = length(Dt_list);
    println("No. of different Dts = ", nt)

    return Dt_list
end


# Calculates characteristic timescales for diffusion, advection, loss, and reacceleration processes
# Inputs: p_list = momentum grid points, z_min = minimum axial grid value, Dz = function for axial diffusion coefficient, Dp = function for momentum diffusion coefficient, p_dot = function for momentum loss rate, v_w = wind speed for advection, n = gas density for losses
# Outputs: min_tau_diff = minimum diffusion timescale, min_tau_adve = minimum advection timescale, min_tau_loss = minimum loss timescale, min_tau_reac = minimum reacceleration timescale, Dt_min = recommended minimum timestep for stability
function make_tau(p_list::Vector{Float64}, z_min::Float64, Dz::Function, Dp::Function, p_dot::Function, v_w::Float64, n::Float64)
    ...
    tau_adve = (z_min / v_w) .* ones(length(p_list)) # Time for p to advect across z_min with a wind speed of v_w
...
# Computes source/injection term arrays Q at grid points and Qu at upper momentum points
# Inputs: r_list = radial grid points, z_list = axial grid points, p_list = momentum grid points, pu_list = upper momentum grid points, Qf = function for source term Q(r, z, p)
# Outputs: Q = source term array at (r_i, z_j, p_k), Qu = source term array at (r_i, z_j, p_k+1)
...
# Constructs tridiagonal matrices for implicit radial diffusion solver using Crank-Nicolson scheme
# Inputs: r_list = radial grid points, z_list = axial grid points, p_list = momentum grid points, Dr = function for radial diffusion coefficient, Dt = timestep
# Outputs: A_nextr = tridiagonal matrices for next timestep, A_nr = tridiagonal matrices for current timestep
function make_Ar(r_list::Vector{Float64}, z_list::Vector{Float64}, p_list::Vector{Float64}, Dr::Function, Dt::Float64)
    ...
    # Parallelize with threads on outer loop (p); SIMD vectorize inner loop (z)
...
# Computes right-hand side vector for radial diffusion using current distribution and source term
# Inputs: Ar = tridiagonal matrices for radial diffusion, prev = particle distribution at current timestep, Q_arr = source term array, Dt = timestep
# Outputs: B_r = right-hand side array for radial diffusion solver
...
# Constructs tridiagonal matrices for implicit axial diffusion and advection solver using Crank-Nicolson scheme
# Inputs: r_list = radial grid points, z_list = axial grid points, p_list = momentum grid points, Dz = function for axial diffusion coefficient, v_z = function for axial advection velocity, Dt = timestep
# Outputs: A_nextz = tridiagonal matrices for next timestep, A_nz = tridiagonal matrices for current timestep
function make_Az(r_list::Vector{Float64}, z_list::Vector{Float64}, p_list::Vector{Float64}, Dz::Function, v_z::Function, Dt::Float64)
    ...
    # Parallelize with threads on outer loop (p); SIMD vectorize inner loop (r)
    Threads.@threads for k in eachindex(p_list)
...
# Computes right-hand side vector for axial diffusion and advection solver using current distribution and source term
# Inputs: Az = tridiagonal matrices for axial diffusion and advection, prev = particle distribution at current timestep, Q_arr = source term array, Dt = timestep
# Outputs: B_z = right-hand side array for axial diffusion and advection solver
...
# Constructs tridiagonal matrices for implicit momentum loss solver using Crank-Nicolson scheme
# Inputs: r_list = radial grid points, z_list = axial grid points, p_list = momentum grid points, P_dot = function for momentum loss rate, Dt = timestep
# Outputs: A_nextp = tridiagonal matrices for next timestep, A_np = tridiagonal matrices for current timestep
function make_Ap(r_list::Vector{Float64}, z_list::Vector{Float64}, p_list::Vector{Float64}, P_dot::Function, Dt::Float64)
    ...
    # Parallelize with threads on outer loop (z); SIMD vectorize inner loop (r)
...
# Computes right-hand side vector for momentum loss solver using current distribution and source term
# Inputs: Ap = tridiagonal matrices for momentum loss, prev = particle distribution at current timestep, Q_arr = source term array at p_k, Qu_arr = source term array at p_k+1, Dt = timestep
# Outputs: B_p = right-hand side array for momentum loss solver
...
# Constructs tridiagonal matrices for momentum reacceleration (diffusion in momentum space) solver using Crank-Nicolson scheme
# Inputs: r_list = radial grid points, z_list = axial grid points, p_list = momentum grid points, Dp = function for momentum diffusion coefficient, Dt = timestep
# Outputs: A_nextm = tridiagonal matrices for next timestep, A_nm = tridiagonal matrices for current timestep
...
# Computes right-hand side vector for momentum reacceleration solver using current distribution and source term
# Inputs: Am = tridiagonal matrices for momentum reacceleration, prev = particle distribution at current timestep, Q_arr = source term array at p_k, Dt = timestep
# Outputs: B_m = right-hand side array for momentum reacceleration solver
